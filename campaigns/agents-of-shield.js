...
(function (global) {
  ...
  global.registerCampaign({
    ...
    helpDe: [
      "Die Tabelle „Verbleibende Geheimnismarker nach Szenario“ hält drei Direktoriumsmitglieder mal vier Szenarien fest. Der Spielaufbau jedes Szenarios legt genau so viele Marker wieder aus, wie hier für das vorige stehen — deshalb ist ein leeres Feld etwas anderes als eine eingetragene Null, und der Druck zeigt es als Strich.",
      "Die Szenarien reichen einander Zahlen weiter: Szenario 1 hält die Schergen und Nebenpläne im Spiel fest, die Szenario 2 als Alarmstufe einliest, und Szenario 2 die befreiten Gefangenen, die Szenario 3 in Schlossmarker umrechnet.",
      "Die vier Adaptoid-Kästchen von Szenario 3 heißen nicht „besiegt“, sondern „war am Ende noch im Spiel“ — genau die legt Szenario 5 wieder aus.",
      "Szenario 4 sammelt die überlebenden Thunderbolts: kein Scherge überlebt zweimal, deshalb schließt ein gewählter Name sich für die anderen Zeilen. Die Liste führt Namen, keine Karten — „Atlas“ ist auf zwei verschiedenen Karten gedruckt, und der Bogen verlangt den Namen.",
      "Der große Block sind die Beweiskombinationen: 27 gedruckte Zeilen zum Durchstreichen, alle Kombinationen aus drei Mitteln, drei Motiven und drei Gelegenheiten. Das Regelheft sagt, gestrichen wird nach den Symbolen der erhaltenen Beweiskarten — damit hängt jede Durchstreichung allein an diesen neun Karten, und genau die werden hier angehakt. Die Tabelle darunter wird daraus gerechnet; an ihr wird nichts einzeln eingetragen.",
      "Daraus fällt die Pointe der Kampagne: je Gruppe liegt genau eine der drei Karten im A.I.M.-Umschlag. Sind also zwei erhalten, ist die dritte die des Maulwurfs — ihr Kästchen wird zugemacht, dort gibt es nichts mehr einzutragen, und ihr Name steht rot da.",
      "Das Zumachen ist einseitig: geschlossen wird nur ein Kästchen ohne Häkchen. Ein Bogen, der mit allen drei Karten einer Gruppe ankommt — im Spiel unmöglich —, friert deshalb nicht ein: drei sind nicht zwei, also schließt nichts, und die Tabelle streicht sich vollständig durch, was von allein sagt, dass etwas nicht stimmt.",
      "Das gelbe „Notizen“ oben ist der Titel der Zählertabelle, kein Feld.",
    ],
    helpEn: [
      "The table “Remaining Secret Counters by Scenario” records three board members by four scenarios. Every scenario’s setup puts back exactly as many counters as are recorded here for the previous one — which is why an empty cell is a different thing from a recorded zero, and the printout shows it as a dash.",
      "The scenarios hand each other numbers: scenario #1 records the minions and side schemes in play, which scenario #2 reads back as its Alert Level, and scenario #2 records the rescued captives, which scenario #3 turns into lock counters.",
      "The four Adaptoid boxes in scenario #3 do not mean “defeated” but “still in play at the end” — those are the ones scenario #5 puts back into play.",
      "Scenario #4 collects the surviving Thunderbolts: no minion survives twice, so a name already chosen is closed to the other rows. The list holds names rather than cards — two different cards are printed “Atlas”, and the sheet asks for the name.",
      "The big block is the evidence combinations: 27 printed rows to cross out, all the combinations of three means, three motives and three opportunities. The rulebook says the crossing out follows the icons on the evidence cards gained — so every strike follows from those nine cards and nothing else, and those nine are what is ticked here. The table below is computed from them; nothing in it is entered row by row.",
      "Out of that falls the campaign’s payoff: each group has exactly one of its three cards sealed in the A.I.M. envelope. Once two have been gained the third is the mole’s — its box is closed, there being nothing left to record there, and its name is called out in red.",
      "The closing is one-sided: only a box without a tick is ever closed. A sheet arriving with all three cards of a group — impossible in play — therefore does not freeze: three is not two, so nothing closes, and the table crosses itself out entirely, which says by itself that something is off.",
      "The yellow “Notes” at the top is the title of the counter table, not a field.",
    ],
