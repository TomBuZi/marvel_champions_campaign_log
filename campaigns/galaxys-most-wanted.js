/* Marvel Champions — "The Galaxy's Most Wanted" (MC16) campaign log.
   ...
   THE BALANCE ALSO CLOSES THE CARDS IT CANNOT PAY FOR. A Market card costing
   more than the player has left is disabled in their list and drawn red and
   italic, on top of the group rule that already closes what somebody else
   bought. The budget is per ROW rather than per player: a row that already
   holds a card gets that card's cost added back, because swapping a 4 for a 6
   costs 2 and not 6. Without that, a player who spent everything could not even
   re-pick what they are already holding.

   THIS LOCK CANNOT CORNER ANYONE, which is the MC60/MC32/MC40/MC45 rule applied
   to a select instead of a checkbox. A row's OWN card is never disabled, so an
   overspent sheet arriving from an import or an old #log= link stays operable;
   the placeholder is never disabled, so a row can always be emptied; and the
   earned figure above is never restricted, so the way out is always on screen.
   And where the budget is unknown — nothing entered under "units earned" —
   nothing is locked at all, because "not recorded" is not the same statement as
   "nothing left".
...
(function (global) {
    ...
    state.players.forEach(function (player, i) {
      ...
      card.appendChild(fieldRow(t("colUnitsEarned"), W.numberField({
        ...
        onChange: function (next) {
          ...
          ctx.save();
          paintUnits(t, state);
          /* The budget moved, so what this player can afford moved with it. */
          paintMarket(t, state);
        },
      })));
  ...
  function renderMarket(t, lang, state) {
    ...
    section.appendChild(grid);
    /* A greyed-out option says "no" without saying why, and there are two
       different reasons here. One line naming both beats leaving the reader to
       work out which applies. */
    var rule = W.el("p", "hint");
    rule.textContent = t("marketLocks");
  ...
  function paintMarket(t, state) {
    var selects = document.querySelectorAll("[data-market-select]");
    if (!selects.length) return;
    /* First, because syncUnique() ASSIGNS `disabled` rather than adding to it:
       it re-enables everything nobody has taken. Anything else that wants a
       card closed has to come after it and may only ever close, never open. */
    ...
    /* And what the player cannot pay for. Runs per select rather than per
       player, because the budget of a row includes the card the row is already
       holding — swapping a 4 for a 6 costs 2. See the file header for why this
       can never corner anybody. */
    Array.prototype.forEach.call(selects, function (select) {
      var at = String(select.getAttribute("data-market-select")).split("-");
      var player = state.players[Number(at[0])];
      var held = player ? poolEntry(MARKET, select.value) : null;
      /* No budget recorded means no budget known, so nothing is locked. */
      var budget = (player && player.unitsEarned != null)
        ? unitsLeft(player) + (held ? held.cost : 0)
        : null;

      Array.prototype.forEach.call(select.options, function (option) {
        var entry = option.value ? poolEntry(MARKET, option.value) : null;
        /* Never the placeholder, and never the row's own card: those two are
           the whole of the way back out. */
        var dear = !!entry && budget != null && entry.cost > budget &&
          option.value !== select.value;
        option.classList.toggle("too-dear", dear);
        /* Only ever closes. Uniqueness had its say above and keeps it. */
        if (dear) option.disabled = true;
      });
    });
  }
  ...
  global.registerCampaign({
    ...
    helpDe: [
      "Die Units sind die Währung der Guardians: sie werden nach jedem Szenario notiert und zwischen den Szenarien für Marktkarten ausgegeben. Deshalb stehen sie auf beiden Stufen — die Expertenregeln geben ihnen nur eine weitere Verwendung.",
      "Hier weicht der Bogen bewusst vom Papier ab. Gedruckt ist ein Feld „Übrige Units“, ein Kontostand, den man mit dem Bleistift bei jedem Kauf ausradiert. Eingetragen werden hier stattdessen die „Verdienten Units“, und die übrigen werden daraus gerechnet: verdient minus die Kosten der Marktkarten, die daneben stehen. Der Bogen kennt diese Kosten ohnehin, und eine „übrige“ Zahl, die den Karten danebensteht und ihnen still widerspricht, wäre ein Fehler, den niemand sieht.",
      "Wird mehr ausgegeben als verdient, steht die Zahl rot da, statt bei null zu stoppen: Überziehen ist ein Fehler, auf den man zeigen will.",
      "Eines kann die Rechnung nicht sehen: auf Expertenstufe darf jeder Spieler beim Aufbau der Szenarien 2 bis 5 eine Unit ausgeben, um seine Identität zu heilen, und das hält der Bogen nirgends fest. Solange der Expertenhaken gesetzt ist, sagt der Spielerbereich das auch.",
      "Marktkarten sind Auswahlfelder über die 28 gedruckten Karten, nach Unit-Kosten sortiert und mit den Kosten hinter jedem Namen. Von jeder Karte darf die Gruppe während einer Kampagne nur ein Exemplar verwenden: eine gekaufte Karte ist deshalb bei allen anderen Spielern gesperrt — ein Spieler darf aber beliebig viele halten.",
      "Was der Spieler nicht bezahlen kann, ist ebenfalls gesperrt und steht rot und kursiv in der Liste. Gerechnet wird je Zeile: eine Zeile, die schon eine Karte hält, bekommt deren Kosten gutgeschrieben, denn eine 4 gegen eine 6 zu tauschen kostet 2 und nicht 6. Wo nichts unter „Verdiente Units“ steht, wird auch nichts wegen des Preises gesperrt: nicht notiert ist nicht dasselbe wie nichts übrig.",
      "Einträge unter „Karten in der Sammlung“ lassen sich zwischen den Spielerspalten ziehen, weil Szenario #3 jeden Spieler genau seine eigenen notierten Karten aus dem Spiel entfernen lässt.",
      "Die vier „Galaktische Artefakte“-Nebenpläne sind Kästchen über dem Satz und keine numerierten Zeilen: der Bogen numeriert zwar vier Zeilen, aber die Nummer trägt nichts — Szenario #4 liest sie als Menge zurück.",
      "„Kontrolle des Machtsteins“ speichert den Spielerplatz und zeigt den aktuellen Namen, damit eine später korrigierte Identität die Eintragung nicht veralten lässt.",
      "Zwei Zeilen werden nur gerechnet, nie gespeichert und nicht mitgedruckt: neben den Ausweichmarkern die zusätzliche Bedrohung, die Szenario #5 daraus auf „Zangenmanöver“ legt (3 minus die Zahl, mindestens 0), und unter den vier Kopfgeldjäger-Kästchen die Karten, die die aktuelle Zahl der Kreuze freischaltet — samt dem Szenario, ab dem jede geprüft wird, denn die Staffel wächst mit dem Szenario.",
    ],
    helpEn: [
      "Units are the Guardians’ currency: they are recorded after every scenario and spent between scenarios on Market cards. That is why they sit at both difficulties — the expert rules only give them one more use.",
      "Here the sheet departs from the paper on purpose. The print has an “Unspent Units” box, a running balance rubbed out with every purchase. What is entered here instead is “Units earned”, and the unspent figure is computed from it: earned minus the cost of the Market cards standing beside it. The sheet knows those costs anyway, and an “unspent” number that sits next to the cards and silently disagrees with them would be a bug nobody can see.",
      "Spend more than you earned and the figure goes red rather than stopping at zero: overspending is a mistake worth pointing at.",
      "One thing the sum cannot see: at expert level the setup of scenarios #2 to #5 lets each player spend a unit to heal their identity, and nothing on the sheet records that they did. While the expert switch is on, the player area says so.",
      "Market cards are selects over the 28 printed cards, grouped by unit cost and with the cost behind every name. Only one copy of each may be used by the players as a group during a campaign: a card someone has bought is therefore closed to all the others — though one player may hold as many as they like.",
      "What a player cannot pay for is closed as well, drawn red and italic in the list. The budget is worked out per row: a row already holding a card gets that card’s cost credited back, because swapping a 4 for a 6 costs 2 and not 6. Where nothing is entered under “Units earned”, nothing is locked for price either: not recorded is not the same as nothing left.",
      "Entries under “Cards in The Collection” can be dragged between the player columns, because scenario #3 has each player remove exactly their own recorded cards from the game.",
      "The four Galactic Artifacts side schemes are boxes over the set rather than numbered lines: the sheet does number four lines, but the number carries nothing — scenario #4 reads them back as a set.",
      "“Power Stone Control” stores the player’s seat and shows their current name, so correcting an identity later does not leave the record stale.",
      "Two lines are computed only, never stored and never printed: beside the evasion counters, the extra threat scenario #5 puts on Pincer Maneuver because of them (3 minus the number, floored at zero), and under the four Headhunter boxes, the cards the current number of marks unlocks — each with the scenario it is first checked from, because the ladder grows with the scenario.",
    ],
    ...
    i18n: {
      de: {
        ...
        marketLocks: "Gesperrt ist, was ein anderer Spieler schon gekauft hat — und rot, was mehr kostet als die übrigen Units des Spielers. Die eigene Karte einer Zeile bleibt immer wählbar.",
      ...
      en: {
        ...
        marketLocks: "Cards another player has already bought are closed — and cards dearer than this player's unspent units are red. A row's own card always stays selectable.",
