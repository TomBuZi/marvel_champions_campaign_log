...
(function (global) {
  ...
  global.registerCampaign({
    ...
    helpDe: [
      "Die vier Zeilen der Tabelle sind die Mission-Nebenpläne, und ihre drei Textspalten — Spielaufbau, Besiegt, Nicht besiegt — sind gedruckter Regeltext, an dem nichts eingetragen wird. Eingetragen wird das Ergebnis, und zwar in den beiden Ergebnisspalten selbst.",
      "Auf dem Papier steht dort kein Kästchen: die Regel sagt, man solle den Namen durchstreichen und dann die Anweisung daneben befolgen. Das Häkchen IST dieses Durchstreichen, und weil es in der Spalte sitzt, hält es zugleich fest, welcher der beiden Zweige gilt — ein Bit mehr, als das Papier führt. Alles in diesen Spalten gilt „für den Rest der Kampagne“, das Papier erwartet also, dass man sich erinnert; ein Logbuch sollte das nicht.",
      "Beide Kästchen schließen einander aus: eine Mission ist entweder besiegt oder nicht. Der durchgestrichene Name ist dabei abgeblendet, die Zeile nicht — der Text der Ergebnisspalten bleibt gerade deshalb in Kraft, weil die Mission gespielt wurde.",
      "Die fünf Kästchen darunter sind die Aufseher-Schergen, und dort kehrt sich die Leserichtung um: die Überschrift heißt „Verfügbare Aufseher-Schergen“, das Häkchen bedeutet aber „nicht mehr verfügbar“.",
      "Die beiden Markierungen des Bogens bedeuten auch nicht dasselbe: ein durchgestrichener Missionsname heißt „war im Spiel, also verbraucht“ — unabhängig vom Ergebnis —, ein durchgestrichener Scherge heißt „wurde besiegt“.",
    ],
    helpEn: [
      "The four rows of the table are the MISSION side schemes, and their three text columns — Setup, Defeated, Not Defeated — are printed rule text with nothing to enter. What is entered is the outcome, and it is entered in the two outcome columns themselves.",
      "The paper prints no box there: the rule says to strike out the name and then follow the instruction beside it. The tick IS that strike, and because it sits in the column it also records which of the two branches applied — one bit more than the paper carries. Everything in those columns holds “for the rest of the campaign”, so the paper expects you to remember which one you followed; a log should not have to.",
      "The two boxes exclude each other: a mission is either defeated or it is not. The struck name is dimmed and the row is not — the text of the outcome columns stays in force precisely because the mission was played.",
      "The five boxes below are the OVERSEER minions, and there the reading turns around: the heading says “Available Overseer minions”, but the tick means “no longer available”.",
      "The sheet’s two marks do not mean the same thing either: a struck mission name means “it was in play, so it is spent” — whatever the outcome — while a struck minion means “it was defeated”.",
    ],
