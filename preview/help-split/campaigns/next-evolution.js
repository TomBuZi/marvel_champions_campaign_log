...
(function (global) {
  ...
  global.registerCampaign({
    ...
    helpDe: [
      "„Besiegte Marauders“ druckt genau drei numerierte Zeilen, und drei ist nicht geraten: Szenario 1 ist gewonnen, wenn drei der sieben Marauder besiegt sind. Weil die Zeilen numeriert sind, ist die Position Teil der Eintragung — und ein Schurke, der schon in einer Zeile steht, ist in den anderen abgeblendet.",
      "„Gerettete Morlocks“ ist eine Zahl ohne Obergrenze. Der Kartensatz enthält vier Morlocks und „Mutanten-Massaker“ gibt jedem Spieler einen, im Solospiel zwei — gedruckt ist aber keine Grenze, und eine Grenze, die niemand gedruckt hat, kann nur falsch sein.",
      "„Schaden auf Hope Summers“ hat zwei Felder, weil der Bogen zwei Zeilen druckt: Szenario 3 und Szenario 4. Die anderen drei halten über sie nichts fest, deshalb steht dort kein Feld — auch kein abgeblendetes.",
      "Im Gitter darunter sind drei der fünf Spalten gedruckter, unveränderlicher Text: der Nebenplan, die Begegnungskarte und die Umgebung. Eingetragen werden nur „Szenarionummer“ und „Verdient?“.",
      "Sechs gedruckte Zeilen stehen fünf Szenarien gegenüber, denn jedes Szenario wählt einen Nebenplan, „der noch nicht gewählt wurde“ — eine Zeile bleibt am Ende zwangsläufig leer, und das ist der Bogen, keine Lücke darin. Ein Szenario, das in einer Zeile steht, ist in den übrigen abgeblendet.",
      "Das Häkchen „Verdient?“ bleibt zu, solange die Zeile kein Szenario trägt: verdient wird eine Umgebung, indem der gewählte Nebenplan besiegt wird, und ein nie gewählter war nie im Spiel.",
      "Nicht markiert wird dagegen eine Zeile mit Szenario ohne Häkchen. Nach dem Regelheft wäre dieser Nebenplan aus der Kampagne heraus — aber erst, nachdem das Szenario gewonnen ist, und dieser Bogen hält gar keinen Fortschritt fest. Er kann „noch nicht“ und „endgültig“ nicht auseinanderhalten, also behauptet er es auch nicht.",
    ],
    helpEn: [
      "“Marauders Defeated” prints exactly three numbered lines, and three is not a guess: scenario 1 is won by defeating three of the seven Marauders. Because the lines are numbered, the position is part of the entry — and a villain already standing in one line is greyed out in the others.",
      "“Morlocks Saved” is a number with no ceiling. The card set holds four Morlocks and “Mutant Massacre” hands each player one, two in a solo game — but no bound is printed, and a bound nobody printed can only be wrong.",
      "“Hope Summers’s Damage” has two fields because the sheet prints two lines: scenario 3 and scenario 4. The other three record nothing about her, so there is no field for them — not a disabled one either.",
      "In the grid below, three of the five columns are printed, fixed text: the side scheme, the encounter card and the environment. Only “Scenario Chosen” and “Earned?” are entered.",
      "Six printed rows stand against five scenarios, because each scenario picks a side scheme “that has not been chosen previously” — one row must stay empty at the end, and that is the sheet rather than a gap in it. A scenario standing in one row is greyed out in the rest.",
      "The “Earned?” box stays closed while its row carries no scenario: an environment is earned by defeating the chosen side scheme, and one that was never chosen was never in play.",
      "What is not marked is a row with a scenario and no tick. By the rulebook that side scheme would be out of the campaign — but only once the scenario has been won, and this sheet records no progress at all. It cannot tell “not yet” from “for good”, so it does not claim to.",
    ],
