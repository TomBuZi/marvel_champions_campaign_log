...
(function (global) {
  ...
  global.registerCampaign({
    ...
    helpDe: [
      "Der Abschnitt für den Siegpunktestapel druckt keine Spalten, der für das Begegnungsdeck vier — und genau das ist der Unterschied: Entfernen ist endgültig und gilt für die ganze Kampagne, deshalb eine Menge; das Begegnungsdeck wird im nächsten Aufbau wieder eingemischt, deshalb vier Spalten, und dieselbe Karte darf in mehreren stehen.",
      "Das Deck-Gitter steht hier als Tabelle, Karten links und Szenarien oben — gegenüber dem Druck gedreht, weil aus zwanzig wiederholten Kartennamen so fünf plus vier Spaltenköpfe werden.",
      "Die Rollen-Upgrades stehen in der Spielerkarte direkt unter der Rolle, denn die Rolle entscheidet, welche fünf Verbesserungen überhaupt erscheinen. Ohne gewählte Rolle steht dort der Hinweis, erst eine zu wählen.",
      "Jeder Spieler muss eine andere Rolle nehmen, deshalb ist eine vergebene Rolle bei den anderen abgeblendet. Wer die Rolle wechselt, verliert nichts: die Eintragungen der alten Rolle bleiben gespeichert und werden nur nicht mehr gezeigt.",
      "Wie viele Verbesserungen die Kampagne schon vergeben hat, zählt der Bogen nicht mit: die Regel begrenzt eine Anzahl und keine bestimmte Karte, das Papier prüft es auch nicht, und ein frischer Bogen wäre sonst sofort fast ganz zu.",
      "Bei Jubilee sperren sich „im Spiel“ und „aus Kampagne entfernt“ innerhalb eines Szenarios gegenseitig — der Sieg-Schritt sagt genau das. Die Szenarien hängen außerdem aneinander: die Kästchen eines Szenarios öffnen erst, wenn das vorige „im Spiel“ trägt, und sobald sie entfernt ist, sind alle späteren zu. Szenario 2 hat nichts davor und ist immer offen.",
      "Ebenso im Gitter: eine Karte im Siegpunktestapel ist aus der Kampagne entfernt, ihre ganze Zeile im Begegnungsdeck ist deshalb durchgestrichen und zu. Eine Karte kann in einem frühen Szenario im Begegnungsdeck gestanden haben und zwei Szenarien später im Siegpunktestapel landen — der Bogen druckt dort keine Spalten und kann deshalb nicht sagen, wann sie entfernt wurde.",
    ],
    helpEn: [
      "The victory-display section prints no columns and the encounter-deck one prints four — and that difference is the point: removal is permanent and holds for the whole campaign, so it is one set; the encounter deck is shuffled back in at the next setup, so it keeps four columns, and the same card may stand in several of them.",
      "The deck grid is a table here, cards down the side and scenarios across the top — transposed against the print, because that turns twenty repeated card names into five plus four column labels.",
      "The role upgrades sit in the player card directly under the role, because the role decides which five upgrades appear at all. With no role chosen there is a line saying to pick one first.",
      "Each player must take a different role, so a role already taken is greyed out for the others. Changing role costs nothing: what was entered under the old one stays stored and is only no longer shown.",
      "The sheet does not count how many upgrades the campaign has granted: the rule caps a number rather than naming a card, the paper does not check it either, and a fresh sheet would otherwise arrive almost entirely shut.",
      "For Jubilee, “in play” and “removed from campaign” close each other within one scenario — the victory step says exactly that. The scenarios also hang together: a scenario’s boxes open only once the one before it carries “in play”, and as soon as she is removed every later one closes. Scenario 2 has nothing before it and is always open.",
      "The same in the grid: a card in the victory display is removed from the campaign, so its whole encounter-deck row is struck through and closed. A card can have sat in an early scenario’s encounter deck and land in the victory display two scenarios later — the sheet prints no columns there and so cannot say when it was removed.",
    ],
