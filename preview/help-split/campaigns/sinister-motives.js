...
(function (global) {
  ...
  global.registerCampaign({
    ...
    helpDe: [
      "Auf dem gedruckten MC27-Bogen ist kein einziges Häkchen — alle Felder sind Zeilen zum Hineinschreiben. Wo die Zeile eine Karte aus einem gedruckten Satz meint, steht hier der Satz: Community Service und Last Ones Standing fragen, welche Karten, deshalb Kästchen über den fünf Nebenschemata beziehungsweise den sechs Schurken der Sinister Six.",
      "Der Reputationsabschnitt hat auf dem Logbuchblatt kein Gegenstück — die laufende Reputation lebt auf der Leiste der ersten Seite. Hier steht dafür ein Zahlenfeld von 0 bis 35 und daneben die sieben Stufen (1, 5, 9, 13, 17, 21, 25) mit Belohnung und Strafe; erreichte Stufen sind hervorgehoben, die noch offenen abgeblendet.",
      "Dieselbe Zahl schaltet die Felder frei: S.H.I.E.L.D. Tech ab 1, Aspect Advantage ab 9, Planning Ahead ab 17, die drei Osborn-Tech-Zellen ab 1, 13 und 21. Ein gesperrtes Feld sagt, ab welcher Stufe es aufgeht, und wird nie geleert: wer die Reputation nach einem Zahlendreher zurückstellt, verliert keine Eintragung.",
      "Osborn Tech wird je Stufe einmal gezogen und behält deshalb eine Zelle je Stufe; jede Karte gibt es nur einmal, eine gewählte verschwindet aus den anderen Zellen.",
      "Die beiden ausgelosten Felder haben einen Würfel, und der ist genau dann bedienbar wie das Feld selbst. Bei Osborn Tech zieht er eine Karte aus denen, die in keiner anderen Zelle stehen. Bei S.H.I.E.L.D. Tech teilt er drei Verbesserungen aus, von denen eine behalten wird — die Auslosung steht nur bis zur Wahl und wird nicht gespeichert, sie ist keine Eintragung auf dem Bogen. Ein Würfel überschreibt nie eine vorhandene Wahl.",
      "Die drei Belohnungsfelder druckt der Bogen als eigene Blöcke mit einer Spalte P1 bis P4. Diese Spalte ist nur die Spielerliste noch einmal, deshalb stehen die Felder hier direkt in der Spielerkarte.",
    ],
    helpEn: [
      "The printed MC27 sheet carries no checkbox at all — every field is a line to write on. Where the line means a card from a printed set, the set is here: Community Service and Last Ones Standing ask which cards, so they are boxes over the five side schemes and over the six villains of the Sinister Six.",
      "The reputation section has no counterpart on the log page — the running reputation lives on the track on page one. Here it is a number field from 0 to 35, and beside it the seven rungs (1, 5, 9, 13, 17, 21, 25) with their reward and penalty; the rungs reached are highlighted, the ones still ahead dimmed.",
      "That same number opens the fields: S.H.I.E.L.D. Tech at 1, Aspect Advantage at 9, Planning Ahead at 17, and the three Osborn Tech cells at 1, 13 and 21. A closed field says which rung opens it, and is never cleared: putting the reputation back after a mistyped number costs no record.",
      "Osborn Tech is drawn once per rung and therefore keeps one cell per rung; each card exists once, and choosing one removes it from the other cells.",
      "The two drawn fields have a die each, usable exactly when the field is. The Osborn Tech die draws a card from those no other cell holds. The S.H.I.E.L.D. Tech die deals three upgrades to keep one of — that draw stands only until the choice is made and is not stored, since it is not an entry on the sheet. A die never overwrites a choice already made.",
      "The three reward fields are printed as their own blocks with a P1 to P4 column. That column is only the player list again, so the fields sit directly in the player card here.",
    ],
