...
(function (global) {
  ...
  global.registerCampaign({
    ...
    helpDe: [
      "Die fünf Szenarien werden in fester Reihenfolge gespielt, deshalb hält der Bogen nur fest, was von einem ins nächste mitgeht. Wo auf Papier eine leere Zeile steht, stehen hier die tatsächlichen Karten: vier gedruckte Möglichkeiten je Feld, und jedes Feld trägt seine eigene Regel.",
      "Verpflichtungen hat jeder Spieler für sich — zwei Spieler dürfen also dieselbe halten. Tech- und Basis-Verbesserung gibt es dagegen je einmal in der ganzen Kampagne: eine gewählte Karte verschwindet aus den Feldern der anderen Spieler.",
      "Gerettete Verbündete gibt es ebenfalls nur je einmal, aber ein Spieler kann mehrere haben — deshalb Kästchen statt Auswahlfeld. Bei den übrigen Spielern sind sie gesperrt statt verschwunden, und der Sperrgrund nennt, wer den Verbündeten hat.",
      "Bei den Experimentalwaffen aus Szenario 1 will der Bogen die Namen, nicht die Anzahl: Szenario 2 mischt genau diese Karten wieder ins Begegnungsdeck.",
      "Nach Szenario 4 fragt der Bogen, welche Spieler mit Handlangern im Gefecht waren. Auf Papier ist das eine Zeile zum Hineinschreiben, hier ein Häkchen je Spieler — die Mitspieler kennt die App ohnehin.",
    ],
    helpEn: [
      "The five scenarios are played in a fixed order, so the sheet records only what carries from one into the next. Where the paper has a blank line, this has the actual cards: four printed possibilities per field, and each field carries its own rule.",
      "Obligations belong to each player separately — two players may hold the same one. Tech and Basic Upgrade, by contrast, exist once in the whole campaign: a chosen card disappears from the other players’ fields.",
      "Rescued Allies also exist once each, but one player may hold several — hence boxes rather than a dropdown. On the other players they lock rather than disappear, and the reason names who holds the ally.",
      "For the Experimental Weapons out of scenario 1 the sheet wants the names, not a count: scenario 2 shuffles exactly those cards back into the encounter deck.",
      "After scenario 4 the sheet asks which players were engaged with minions. On paper that is a line to write names on; here it is one box per player, since the app already knows who is playing.",
    ],
